    ...
    <form action="">
      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        <div className='flex-1 max-w-48'>
          ...
          <select onChange={e => setInputs({ ...inputs, roomType: e.target.value })} value={inputs.roomType}
        ...
        <div>
          ...
          <input type="number" placeholder='0' className='border border-gray-300 mt-1 rounded p-2 w-24' value={inputs.pricePerNight} onChange={e => ({ ...inputs, pricePerNight: e.target.value })} />
        </div>
      </div>
      ...
      <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
        {Object.keys(inputs.amenities).map((amenity, index) => (
          <div key={index}>
            <input type="checkbox" id={`amenitiies${index + 1}`} checked={inputs.amenities[amenity]} onChange={() => setInputs({ ...inputs, amenities: { ...inputs.amenities, [amenity]: !inputs.amenities[amenity] } })} />
            <label htmlFor={`amenitiies${index + 1}`}> {amenity}</label>
          </div>
        ))}
