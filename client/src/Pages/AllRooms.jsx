...
const AllRooms = () => {
    ...
    // Function to check if a room matches to selected room Types
    const matchesRoomType = (room) => {
        return selectedFilters.roomType.length === 0 || selectedFilters.roomType.includes(room.roomType);
    }
    // Function to check if a room matches to selected price ranges
    ...
    const filteredRooms = useMemo(() => {
        return rooms.filter(room => matchesRoomType(room) && matchesPriceRange(room) && filterDestinations(room)).sort(sortRooms)
                ...
                <div className={`${openFilters ? 'h-auto' : 'h-0 lg:h-auto'} overflow-hidden transition-all duration-700`}>
                    ...
                    <div className='px-5 pt-5 pb-7'>
                        ...
                        {sortOptions.map((option, index) => (
                            ...
                            selected={selectedSort===option} onChange={()=>{handleSortChange(option)}} />
                        ))}
                    </div>
                </div>
